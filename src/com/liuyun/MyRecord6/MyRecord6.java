 ...
 * 3.在播放刚录的文件时，需要突然录音，点击录音后自动关闭正在播放的文件，开始录音
 * 4.对停止录音进行改进，将录音按钮设计成类似微信的形式，按下时进行录音，松开即停止录音并保存即可
 * 5.舍弃了ActionListener使用了MouseListener
 * 6.添加了删除当前录音的功能
 * 7.在录音的时候在面板上显示音频波，表示正在录音
 * 8.在播放录音的时候，在面板上显示音频波
 */
/**
 * @autor 刘运
 * 时间：2014-07-02
 * 更新日志：用于测试肖家河工业园区的语音留言测试
 * 修改了路径，不用再D盘下建立AudioFile文件夹，直接在D盘进行交换
 * 因为最终的路径有空格导致格式转换会出现问题，所以利用copyFile来进行复制到最终的文件夹中
 */
...
public class MyRecord6 extends JFrame implements MouseListener{
	...
	public MyRecord6()
	{
		...
		//定义jp1的字体
		Font jpFont = new Font("楷体",Font.BOLD,40);
		jl1 = new JLabel("请留下您想说的话");
		jl1.setFont(jpFont);
		jl1.setForeground(Color.orange);
		jp1.add(jl1);
		//定义按钮上面的字体
		Font btFont = new Font("楷体",Font.BOLD,40);
		captureBtn = new JButton("按住 说话");
		//setForeground可以设置按钮上面字体的颜色
		captureBtn.setForeground(Color.ORANGE);
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(e.getSource().equals(captureBtn))
		{
			...
			//王馨悦的测试路径
			String destPath = "E:/tomcat/Tomcat 6.0/webapps/XIPC/tempFile/";
			//最终路径
			//String destPath = "D:/Program Files/apache-tomcat-6.0.35/webapps/XIPC/tempFile/";
	...
	public void save()
	{
        ...
        try {	
            ...
            try {
				...
				Process p=run.exec("lame -b 16 "+filePath+"/"+file.getName()+" "+tarPath+tarFile.getName()); //16为码率，可自行修改
				//释放进程
